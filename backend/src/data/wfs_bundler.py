...
class WFSDataBundler:
    ...
    def fetch_all_regions(
        ...
        logger.info(f"DataCollector: {self.wfs_timeout}s timeout, {self.max_retries} retries per service")
        if test_mode:
                ...
                for service_name, layers_dict in data_collector.relevant_geospatial_data.items():
                    ...
                    for layer_name, gdf in layers_dict.items():
                        ...
                        if gdf is not None and len(gdf) > 0:
                            self.wfs_data_collection[service_name][layer_name].append(gdf.copy())
    ...
    def _save_single_layer(
        ...
        if add_region_ids and len(self.scope_region_ids) >= len(wfs_data_list):
            for gdf, region_id in zip(wfs_data_list, self.scope_region_ids[:len(wfs_data_list)]):
        ...
        if duplicates_removed > 0:
            ...
            logger.info(f"Saved {len(combined_dedup)} features to layer: {output_layer_name}")
