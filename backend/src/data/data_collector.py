...
class DataCollector:
    ...
    def __init__(
        ...
        :param wfs_timeout: Timeout in seconds for WFS requests (default: 30).
        :param max_retries: Maximum number of retry attempts for failed WFS requests (default: 3).
        """
    ...
    def initialize_wfs_services(self):
        """Initialize WFS service connections with configured timeout."""
        self.wfs_services = {}
        for service in self.wfs_services_raw:
            self.wfs_services[service.name] = WebFeatureService(
                service.url, version=service.version, timeout=self.wfs_timeout
            )
            ...
            for attempt in range(self.max_retries):
                try:
                    ...
                    if attempt < self.max_retries - 1:
                        logger.warning(
                            f"Failed to get data from {wfs_service} (attempt {attempt + 1}/{self.max_retries}): {e}. Retrying..."
                        )
                    else:
                        logger.error(
                            f"Failed to get data from {wfs_service} after {self.max_retries} attempts: {e}"
                        )
                        # Return empty dict for this service to allow processing to continue
