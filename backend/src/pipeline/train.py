...
def train_and_save_models(
    ...
    evaluate(
        "0 – Naive mean",
        train[TARGET], np.full(len(train), global_mean),
        test[TARGET],  np.full(len(test),  global_mean),
    )
    ...
    evaluate(
        "2 – OLS v_train",
        train[TARGET], ols.predict(X_tr2),
        test[TARGET],  ols.predict(X_te2),
    )
    ...
    evaluate(
        "3 – Ridge numeric",
        train[TARGET], ridge_num.predict(X_tr3),
        test[TARGET],  ridge_num.predict(X_te3),
    )
    ...
    evaluate(
        "4 – Ridge + cat",
        train[TARGET], ridge_cat.predict(X_tr4),
        test[TARGET],  ridge_cat.predict(X_te4),
    )
    ...
    lgb_model.fit(
        X_tr_lgb, train[TARGET],
        eval_set=[(X_te_lgb, test[TARGET])],
        callbacks=[lgb.early_stopping(50, verbose=False), lgb.log_evaluation(period=-1)],
    )
    evaluate(
        "5 – LightGBM",
        train[TARGET], lgb_model.predict(X_tr_lgb),
        test[TARGET],  lgb_model.predict(X_te_lgb),
    )
    ...
    save_model_bundle(
        path=model_out_dir,
        models={"ols": ols, "ridge_num": ridge_num, "ridge_cat": ridge_cat, "lgb": lgb_model},
        config=config,
        results=results,
    )
...
def _evaluate(
    ...
    row = dict(
        ...
        train_tail_mae = (
            ...
            if tr_mask.sum() > 0 else float("nan")
        ),
        ...
        test_tail_mae  = (
            ...
            if te_mask.sum() > 0 else float("nan")
        ),
